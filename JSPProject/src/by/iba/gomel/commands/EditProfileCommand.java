...
public class EditProfileCommand implements IActionCommand {
    ...
    public String execute(final SessionRequest request) {
        ...
        editedRecord.setItem(Integer.parseInt(getParameter(request, "item")));
        editedRecord.setFullName(getParameter(request, "fullName"));
        editedRecord.setAddress(getParameter(request, "address"));
        editedRecord.setPhoneNumber(getParameter(request, "phoneNumber"));
        editedRecord.setMail(getParameter(request, "mail"));
        try {
            editedRecord.setBirthDate(format.parse(getParameter(request, "birthDate")));
        } catch (final ParseException e) {
            EditProfileCommand.LOGGER.error(Constants.PARSE_EXCEPTION, e);
        }
        System.err.println(editedRecord);
